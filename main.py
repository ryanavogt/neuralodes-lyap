...
if __name__ == '__main__':
    ...
    xs = []
    num_test = 3
    theta_0s = [0.3, 0.5, 1.0, 1.5, 2.0, 3.0, 0.2, 1.2, 2.5]
    for theta_0i in theta_0s:
        xi = create_data(tmax=T_MAX * 10, dt=dt, theta0=theta_0i)
        n_batches = xi.shape[0] // BATCH_SIZE
        xs.append(xi[:BATCH_SIZE * n_batches])
    x = np.vstack(xs)
    train_loader, test_loader = create_dataloader(x[:-num_test], x[-num_test:])
    errors = {}
    ...
    f = plt.figure()
    # for integrator in ['euler', 'rk4', 'dy_sys', 'simple']:
    for integrator in ['dy_sys', 'simple']:
        hidden = 100
        ...
        hs = [0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1, 5, 10]
        # hs = [0.001, 0.01, 0.1,  1, 10]
        f = plt.figure(1)
        LE_theta = []
        theta_0_list = [0.1, 0.2, 0.5, 1.0, 1.5, 2.0]
        for theta_0 in theta_0_list:
            ...
            for h in hs:
                ...
                le_file = f'LEs/{integrator}_h{h}_eps{eps}_theta0{theta_0}_LEs.p'
                if integrator in ['dy_sys', 'simple']:
                    if os.path.isfile(le_file):
                        LEs, rvals, J = torch.load(le_file, map_location=device)
                    else:
                        x_le = torch.Tensor(create_data(tmax=np.min((LE_points * h, LE_points)), dt=h,
                                                        theta0=theta_0, d_theta0=d_theta_0)).double().to(device)
                ...
                rv_plot = torch.log2(rvals.cpu().diagonal(dim1=-2, dim2=-1)).cumsum(dim=0).detach().cpu()
            ...
            plt.scatter(torch.Tensor(hs).unsqueeze(1), LE_list.sum(dim=-1), c='k', marker='+', alpha=0.5)
        LE_theta = torch.vstack(LE_theta).cpu()
        plt.figure(4)
        plt.scatter(torch.Tensor(theta_0_list).unsqueeze(-1).repeat(1,2), LE_theta)
        plt.xlabel(r'$\theta_0$')
        plt.ylabel('Lyapunov Exponent')
        plt.title(r'LE vs $\theta_0$ for ' + f'{integrator}')
        plt.savefig(f'Figures/LE_v_theta_{integrator}.png')
        plt.close()
